...
class TileApplication:
	...
	def verify_args(self, args):
		...
		if args.location == None and args.assembly_fwi == None:
			raise Exception("Cannot get tile location without --assembly-fwi argument.")
		if args.variant_diffs == None and args.keep == None:
			raise Exception("Cannot get variant diffs without --keep argument.")
		if args.diff_indices == None and args.keep == None:
	...
	def verify_config(self, config_args):
		...
		if "location" in config_args['features'] and not config_args['assembly_fwi']:
			raise Exception("Cannot get location without assembly_fwi in configuration file.")
		if "base_pair_locations" in config_args['features'] and not config_args['assembly_gz']:
			raise Exception("Cannot get base pair locations without assembly_gz in configuration file.")
		if "variant_diffs" in config_args['features'] and not config_args['assembly_gz']:
			raise Exception("Cannot get variant diffs without keep in configuration file.")
	...
	def set_functionality(self, args):
		...
		if args.variant_diffs == None:
			self.functionality['variant_diffs'] = True
		if args.diff_indices == None:
			self.functionality['diff_indices'] = True	
		if args.base_pair_locations == None:
