...
def setup_parsing(app):
    ...
    app.parser.add_argument(
        '--tile-info',
        type=str,
        help="Location of tile names (for PGP, it is called hiq-pgp-info). Always required.")
    app.parser.add_argument(
        '--location',
        type=int,
        nargs='?',
        default=False,
        help="Whether to get tile location. REQUIREMENTS: cat, zgrep, and assembly-fwi")
    app.parser.add_argument(
        '--variant-diffs',
        type=int,
        nargs='?',
        default=False,
        help="Print tile variants (a/t/c/g) diffs (ClustalW output). REQUIREMENTS: zgrep, keep collection, ClustalW.")
    app.parser.add_argument(
        '--diff-indices',
        type=int,
        nargs='?',
        default=False,
        help="Whether to print the tile variants with diff indices. REQUIREMENTS: zgrep, keep collection, ClustalW.")
    app.parser.add_argument(
        '--base-pair-locations',
        type=int,
        nargs='?',
        default=False,
        help="Whether to get base pair locations. REQUIREMENTS: bgzip and assembly-gz)")
    app.parser.add_argument(
        '--assembly-gz',
        type=str,
        nargs='?',
        default=None,
        help="Location of assembly.00.hg19.fw.gz")
    app.parser.add_argument(
        '--keep',
        type=str,
        nargs='?',
        default=None,
        help="Location of keep collection with *.sglf.gz files")
    app.parser.add_argument(
        '--assembly-fwi',
        type=str,
        nargs='?',
        default=None,
        help="Location of assembly.00.hg19.fw.fwi")
    app.parser.add_argument(
        '--write-to-file',
        type=int,
        nargs='?',
        default=False,
        help="Whether to write to a file or print to stdout.")
...
def parse_input(app):
    ...
    if args.location is None or args.variant_diffs is None or args.diff_indices is None or args.base_pair_locations is None:
    ...
    if not all([args.assembly_fwi, args.keep, args.assembly_gz]):
        ...
        print("No data command line arguments specified. Reading from {}...".format(
            CONFIG_FILENAME))
        with open(CONFIG_FILENAME, 'r') as stream:
            data_args = yaml.load(stream)
